...
const localEnv = { ...process.env, XUYAN_LOCAL: true, SNAPSHOT_LOCAL: true, SNAPSHOT_STORE: snapshotStore };
const server = createServer(async (req, res) => {
  try {
    ...
    const response = url.pathname === '/api/chat' ? await handleChat(request, localEnv, { local: true, store: snapshotStore }) : await worker.fetch(request, localEnv);
