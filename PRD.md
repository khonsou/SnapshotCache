...
本文明确产品的四个顶层模块、项目边界和模块之间的关系。当前静态 Demo 只验证表现层与模拟交互；本文中的模型接入、上下文、记忆、数据权限、快照持久化和缓存服务是后续实现要求。
...
详细字段与校验设计见 [快照数据协议设计稿](docs/SNAPSHOT_SPEC.md)。该文档仍是设计稿，机器可读 Schema 和完整示例尚待补齐，不视为已冻结的实现契约。
...
验证项目群聊、侧栏收起、项目管理、独立成员名单、角色操作、快照展示和模拟缓存状态。数据、Agent 回复和管理操作均在本地模拟，尚未验证真实身份权限、模型连接或后端缓存。
