...
本文明确产品的四个顶层模块、项目边界和模块之间的关系。当前版本已接入真实文本模型，并具备本地 dummy 快照、隔离回放和自由对话生成快照基线；实现与验收状态以 docs/STATUS.md 为准。生产首发优先完成 Timeline 真实数据、真实项目和公司 OAuth，允许暂不提供跨会话持久化与缓存；完整产品仍按本文保留持久回放、记忆和缓存目标。
...
现有项目群聊、侧栏、项目与成员管理交互继续保留，项目记录仍在页面内存。文本 Agent 已真实调用模型，自由对话生成快照和 Timeline 真实只读取数已通过本地验收。P1 的 dummy 文件包继续用于协议和隔离回归；DAO OAuth 登录门禁代码已实现，但真实 HTTPS 回调尚未验收，生产项目授权、项目持久化与缓存仍未实现。详见 docs/STATUS.md。

### 生产首发切片

- 保留现有 GUI、快照协议、校验器、隔离 viewer、文本模型和动态快照生成能力。
- 接通 Timeline Agent Support 只读数据源；模型只能通过服务端受控工具取数，结果记录来源、观察时间和版本。
- 生产路径使用真实项目配置，不把硬编码项目、前端自报上下文或 dummy 业务数据当成可信事实。
- 接入公司 OAuth，并由服务端把真实用户映射到项目和数据可见范围。
- 优先适配阿里云预发布与生产环境；业务协议不依赖 D1/R2 或其他单一云产品。
- 首发允许当次请求内生成和展示，暂不保证刷新、跨会话历史或模型离线回放；页面不得暗示已经保存或命中缓存。
- 发布前必须通过数据真实性、只读工具、身份授权、密钥保护、快照完整性和 viewer 隔离阻断测试。

生产首发顺序和退出条件见 `docs/PRODUCTION_RELEASE_PLAN.md`。
