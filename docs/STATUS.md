...
当前自动证据：37 项 Node 测试、24 项浏览器测试（12 个场景 × Chromium / WebKit）、7 个 fixture 校验及 Worker 构建通过。完整 Timeline 上下文会让宿主在每轮对话中提供项目限定的只读工具，由 DeepSeek 结合完整对话自主选择；普通查询保持文本回复，明确生成快照时才进入快照链路，与工作无关的问题仍直接回答。普通对话通过 DeepSeek 官方 Anthropic 兼容接口开放网页搜索，明确时效请求要求先搜索再回答；“今天微博有什么新闻？”已真实返回搜索结果和来源 URL。真实 DeepSeek 已在受控模拟 Timeline 响应上主动调用只读工具并正确回答卡片数量，也已生成合规快照；上下文密码未发送给模型。真实目标看板尚未读取。详情见 `docs/testing/P2_ACCEPTANCE.md`。
