...
当前自动证据：36 项 Node 测试、24 项浏览器测试（12 个场景 × Chromium / WebKit）、7 个 fixture 校验及 Worker 构建通过。真实 Timeline 项目中的“读取／获取最新状态”等自然语言会自动进入只读工具链；与工作无关的问题仍直接交给 DeepSeek。普通对话通过 DeepSeek 官方 Anthropic 兼容接口开放网页搜索，明确时效请求要求先搜索再回答；“今天微博有什么新闻？”已真实返回搜索结果和来源 URL。上下文密码脱敏与用户项目连接链路已用模拟响应验证；真实目标看板尚未读取。详情见 `docs/testing/P2_ACCEPTANCE.md`。
