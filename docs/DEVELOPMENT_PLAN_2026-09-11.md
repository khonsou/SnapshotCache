...
本文件保留早期研发规划。实际状态见 docs/STATUS.md；当前阶段与顺序以 `docs/PRODUCTION_RELEASE_PLAN.md` 为准，本文早期工作量估算和 D1/R2 顺序仅保留为历史背景。
...
用户已提供 Timeline 看板及接入指南，指定看板 ID 为 `cd620e2ebb1739b1`；测试项目暂名“Timeline 看板快照验证”。地址、看板 ID 和密码均由已授权员工在该项目唯一的上下文中管理；宿主在启动每轮 Agent 前将密码替换为临时引用，原值不进入模型提示词、日志或快照。
...
- **项目 Agent 服务：已切换到真实 Agent Runtime。** `/api/chat` 在本地 Node 中启动固定版本 Claude Code，由 DeepSeek 提供推理；请求级 MCP 提供受限项目 HTTP 和快照提交。语义关键词分流、拒绝话术纠错和 prompt 修复循环已删除。目标 Timeline、公司 OAuth、自动摘要和长期记忆仍未验收。
...
完整任务、阻断条件和发布边界见 `docs/PRODUCTION_RELEASE_PLAN.md`。以下为历史顺序，当前不再按此执行：
