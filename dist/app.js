...
function overview(data) {
  return `<div class="metrics">${data.metrics.map((m,i)=>`<div class="metric"><div class="metric-label">${m[0]}</div><div class="metric-value">${m[1]}<span>${m[2]}</span></div><div class="metric-detail ${i===0?'positive':''}">${m[3]}</div></div>`).join('')}</div><div class="chart-title">${data===projects.launch?'每日新增预约':'项目推进趋势'}</div><div class="chart" role="img" aria-label="${data===projects.launch?'每日新增预约人数':'项目完成进度'}总体呈上升趋势">${data.bars.map(h=>`<span class="bar" style="--height:${h}%"></span>`).join('')}</div><div class="chart-axis"><span>09.01</span><span>09.05</span><span>09.10</span></div>`;
}
function panel(data, tab) {
  if(tab==='channels') return `<div class="channel-list">${data.channels.map(([label,value])=>`<div class="channel-row"><span>${label}</span><div class="channel-track"><span style="--width:${value}%"></span></div><b>${value}%</b></div>`).join('')}</div><p class="panel-note">${data===projects.launch?'小红书贡献了近一半的预约。建议先复盘表现最好的内容，再安排下一轮投放。':'以上为本期项目分布，可结合进度安排下一轮工作。'}</p>`;
  if(tab==='tasks') return `<div class="timeline">${data.tasks.map(([date,title,owner,status])=>`<div class="timeline-row"><span class="timeline-date">${date}</span><div>${title}<small>${owner}</small></div><span class="status ${status==='已完成'?'done':''}">${status}</span></div>`).join('')}</div>`;
  if(tab==='risks') return `<div class="risk-list"><div class="risk-item"><strong>${data===projects.launch?'详情页验收窗口较短':'跨团队确认时间较紧'}</strong><span class="risk-level">需要关注</span><p>${data===projects.launch?'9 月 12 日验收，距离正式发布只有 3 天。建议今天先确认移动端核心流程。':'建议在本周内对齐交付范围，为修改留出时间。'}</p></div><div class="risk-item"><strong>${data===projects.research?'部分用户样本仍需补充':'后续工作依赖最终物料'}</strong><span class="risk-level">可提前处理</span><p>${data===projects.research?'还差 2 场访谈，建议尽早联系候补用户。':'请相关负责人提前确认交付清单，减少临近截止时间的往返沟通。'}</p></div></div>`;
  return overview(data);
}
function snapshot(project, tab='overview') {
  const data = projects[project];
  const id = `snapshot-${++serial}`;
  const tabs = [['overview','概览'],['channels',project==='launch'?'渠道表现':'项目分布'],['tasks','项目安排']];
  if(tab==='risks') tabs.push(['risks','关注事项']);
  return `<div class="snapshot-container" data-project="${project}" data-snapshot-id="${id}"><div class="snapshot-header"><div><h2>项目进展</h2><p class="snapshot-subtitle">${data.subtitle}</p></div><button class="icon-button" data-expand aria-label="展开项目快照">${icon('expand')}</button></div><div class="snapshot-tabs" role="tablist" aria-label="快照视图">${tabs.map(([key,label])=>`<button role="tab" id="${id}-tab-${key}" aria-controls="${id}-panel" tabindex="${tab===key?'0':'-1'}" data-tab="${key}" aria-selected="${tab===key}">${label}</button>`).join('')}</div><div class="snapshot-body" id="${id}-panel" role="tabpanel" aria-labelledby="${id}-tab-${tab}" tabindex="0">${panel(data,tab)}</div><div class="snapshot-provenance">示例数据<span>9 月 10 日 · 10:32</span></div></div>`;
}
function initialMessages(project) {
  const data = projects[project];
  if(data.isNew) return emptyConversation();
  if(project==='launch') return `<div class="day-divider">9 月 10 日</div>${message('lu','<p class="message-text">早上好，距离新品发布还有 5 天。我们一起对一下进展？</p>','10:30')}${message('qiao','<p class="message-text">视觉物料已经全部定稿，首轮内容今天可以上线。</p>','10:31')}${message('me','<p class="message-text"><span class="mention">@序言</span> 帮我们看看目前的整体进展。</p>')}${message('agent',`<p class="message-text">${data.summary}</p>${snapshot(project)}`)}`;
  return `<div class="day-divider">9 月 10 日</div>${message('lu',`<p class="message-text">我们对一下「${escapeHtml(data.title)}」的最新进展吧。</p>`,'10:30')}${message('me','<p class="message-text"><span class="mention">@序言</span> 整理一下目前的情况。</p>','10:31')}${message('agent',`<p class="message-text">${data.summary}</p>${snapshot(project)}`)}`;
}
...
document.addEventListener('click',event=>{
  ...
  if(tab){const card=tab.closest('.snapshot-container');card.querySelectorAll('[data-tab]').forEach(b=>{b.setAttribute('aria-selected',String(b===tab));b.tabIndex=b===tab?0:-1;});const body=card.querySelector('.snapshot-body');body.innerHTML=panel(projects[card.dataset.project],tab.dataset.tab);body.setAttribute('aria-labelledby',tab.id);}
