...
项目由用户在页面中创建和删除。Timeline 项目不使用部署级项目或连接注册表：在项目的“上下文”窗口分别填写 `Timeline 地址`、`Timeline 看板 ID`、`Timeline 访问密码`，宿主便会在每轮对话中向 DeepSeek 提供限定到该项目的只读工具。是否调用由模型结合完整对话决定，不依赖末条消息关键词；普通项目查询保持文本回复，只有明确要求生成快照／可视化时才进入快照分支。原始上下文仍是唯一配置；密码仅由宿主调用 Timeline，发给 DeepSeek 的副本会脱敏，且密码不会进入快照。没有 D1/R2 时，项目、对话、上下文和生成结果只保留在当前页面，刷新后消失。
